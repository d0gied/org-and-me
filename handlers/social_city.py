...
async def area_planing(callback_query: types.CallbackQuery):
    ...
    await message.edit_text(text=texts.SOCIAL_AREA_PLANING, reply_markup=keyboards.to_social_city)


@router.callback_query(filters.CallbackData("social_culture"))
async def culture(callback_query: types.CallbackQuery):
    message = callback_query.message
    await message.edit_text(text=texts.SOCIAL_CULTURE, reply_markup=keyboards.to_social_city)


@router.callback_query(filters.CallbackData("social_economy"))
async def economy(callback_query: types.CallbackQuery):
    message = callback_query.message
    await message.edit_text(text=texts.SOCIAL_ECONOMY, reply_markup=keyboards.to_social_city)
...
async def social_policy(callback_query: types.CallbackQuery):
    message = callback_query.message
    await message.edit_text(text=texts.SOCIAL_SOCIAL_POLICY, reply_markup=keyboards.to_social_city)
